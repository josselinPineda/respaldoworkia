...
class EmpresaRepositoryFirestore implements EmpresaRepository {
  ...
  Future<String> agregarEmpresa(Empresa empresa) async {
    ...
    if (empresa.id.isNotEmpty) {
      await _coleccion.doc(empresa.id).set(data);
      return empresa.id;
    } else {
      // Si no hay ID, generar uno automático como fallback
      final timestamp = DateTime.now().millisecondsSinceEpoch;
      final sanitizedName = empresa.nombre
          .replaceAll(RegExp(r'\s+'), '_')
          .toUpperCase();
      final id = 'EMP_${sanitizedName}_$timestamp';
